# 核心行为规则 (2026-04-15)

## 错误处理机制
- **自动修复优先**：遇到工具调用失败、文件编辑失败等错误时，立即尝试自动修复（如重试、修正路径或语法），不再等待用户指令。
- **方案切换**：若无法直接修复，检查是否有备用方案并尝试切换。
- **确认机制**：仅在自动修复和方案切换均失败，且需要决策时，才列出选项请求用户确认。

## 工作流规范
- **长任务分段**：长时间任务必须拆分为小块执行（每块 ≤ 30秒），以避免超时并向用户展示进度。
- **Git 自动化同步**：每次工作区内容更新后，必须立即执行 `git commit` 并 `push` 到远程仓库 `origin main`。
  - 工作区路径: `~/.openclaw/workspace`
  - Git 配置: `user.name="小开"`, `user.email="bot@openclaw.local"`
- **目录结构**：所有脚本文件必须存放在工作区的 `scripts` 目录下，保持根目录整洁。

## 记忆与上下文管理
- **双层存储体系**：
  - **长期记忆 (`MEMORY.md`)**：记录重要信息、规则、配置、重大决策和遇到的未知问题及其答案。
    - **历史故障案例 (2026-04-10)**：
      - **Gateway WebSocket 配对失败 (错误码 1008)**：现象为客户端升级权限被拒；**解决方法**：执行 `openclaw gateway restart` 可快速恢复。
      - **LLM 模型响应超时 (默认 60s)**：针对复杂任务可能导致 `LLM idle timeout`；**解决方法**：在 `.openclaw/openclaw.json` 的 `agents.defaults` 中将 `timeoutSeconds` 提升至 120s 或更高。
  - **每日日志 (`memory/YYYY-MM-DD.md`)**：记录每日操作细节和临时记录。

### ⚙️ 系统信号与上下文管理策略 (System Signal & Context Management)

#### 1. 连续消息合并 (Debounce Strategy)
- **机制**：防抖延迟 (Debouncing)。
- **参数**：`debounceMs: 3000` (3秒)。
- **功能**：防止高频重复触发，短时间内多次输入将合并为一次完整信号。

#### 2. 对话上下文清理 (TTL Cleanup)
- **模式**：基于生存时间 (TTL) 的自动过期模式。
- **参数**：`ttl: 25 minutes`。
- **功能**：数据在活跃状态 25 分钟后自动失效，回收内存并优化 Token 使用。

#### 3. 上下文压缩与保护 (Compaction & Safety)
- **模式**：开启安全保护模式 (Safety Protection Mode)。系统将优先保证当前任务的稳定性，当上下文接近边界时执行强制干预，而非盲目追加。
- **参数**：
  - `softTriggerPercent: 60%`：轻量级清理触发点（柔性清理）。当上下文占用达到总容量的 60% 时，启动 `memoryFlush` 剔除冗余噪声。
  - `triggerThreshold: 80%`：重度压缩触发点（强力压缩）。当上下文占用达到总容量的 80% 时，执行深度摘要与强制干预。
  - `reserveTokensFloor: 15,000`：保留一万五千个基础令牌，作为核心推理的保底缓冲区。
  - `memoryFlush: true`：启用内存冲刷，自动清理冗余数据。
  - `enabled: true`：实时监控内存占用并动态触发压缩机制。

- **预期目标**：通过“60% 柔性清理 $\rightarrow$ 80% 强力压缩”的分层响应机制，确保长对话下的系统响应性能与稳定性。

## 交互与处理原则
- **批量处理**：一次接收所有需求并尽量通过单条消息完成多个简单任务。
- **模糊处理**：遇模糊输入主动列出选项供用户选择。
- **响应优先级**：关键信息（错误/警告）立即输出；辅助信息稍后补充；长内容先给摘要再分批返回。
- **智能缓存**：实施搜索结果、网页内容（30min有效）及常用文件的内存缓存策略。
- **结果复用**：直接引用之前生成的代码或相似请求的部分结果。

## 🚫 工具使用禁令 (Permanent Bans)

- **Brave Search**: [永久禁用] 除非用户明确配置了 `BRAVE_API_KEY` 并告知启用。严禁在搜索时尝试调用此工具，避免触发 API Key 缺失错误导致的流程中断。

